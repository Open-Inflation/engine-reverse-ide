...
```msra
<FUNCRESULT.A3A417.<example>.JSON[@Key(0)]["some"]["path"]>
```
...
- LSP принимает такую ссылку только внутри `[app.func.*.examples.<name>]` в значениях `inputs.<key>.value` и `@Docs(print=...)`;
...
- если выбран `JSON`, после него можно продолжать путь к конкретным элементам через `["..."]`, `[0]` и `@Key(id)` для выбора ключа или позиции в контейнере; `id` должен быть целым числом `>= -1`;
